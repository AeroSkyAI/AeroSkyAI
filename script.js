...
async function askAI() {
    ...
    const apiKey = "YOUR_API_KEY_HERE"; // 👈 તારી સાચી Gemini API Key અહીં લખી દેજે
...
// New Chat બટન પર ક્લિક ઇવેન્ટ (જો ઓલરેડી આ લાઇન હોય તો બીજી વાર ન લખવી)
document.querySelector('.new-chat-btn').addEventListener('click', createNewChat);

function createNewChat() {
    // ૧. નવી ચેટ માટે એક યુનિક ID અને નવો ઓબ્જેક્ટ બનાવો
    const newChatId = Date.now().toString(); // ટાઇમસ્ટેમ્પથી યુનિક ID બનશે
    const newChat = {
        id: newChatId,
        title: "New Chat",
        messages: [] // શરૂઆતમાં મેસેજ ખાલી રહેશે
    };

    // ૨. તારા APP ઓબ્જેક્ટના ચેટ લિસ્ટમાં આ નવી ચેટ ઉમેરો
    if (window.APP && APP.chats) {
        APP.chats.unshift(newChat); // લિસ્ટમાં સૌથી ઉપર ઉમેરાશે
        APP.currentChatId = newChatId; // આ ચેટને કરન્ટ ચેટ બનાવો
    }

    // ૩. ચેટ સ્ક્રીનને આખી ખાલી કરીને વેલકમ મેસેજ સેટ કરો
    const chatContainer = document.getElementById('chat');
    if (chatContainer) {
        chatContainer.innerHTML = ''; 
        
        const welcomeMessage = document.createElement('div');
        welcomeMessage.className = 'ai';
        welcomeMessage.innerText = 'Hello! How can I help you with Aviation AI today?';
        chatContainer.appendChild(welcomeMessage);
    }

    // ૪. સાઇડબારની ચેટ લિસ્ટને રિફ્રેશ કરો જેથી નવું કાર્ડ દેખાય
    ...
    // ૫. મોબાઈલ સ્ક્રીન હોય તો સાઇડબાર ઓટોમેટિક બંધ કરી દો
